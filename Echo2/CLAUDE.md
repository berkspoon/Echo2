...
- **Step 1: Schema + Service** — New `view_configurations` table (view_key UNIQUE, display_name, description, category, config JSONB, updated_by). New `db/view_config_service.py` with `get_view_config()` (cached), `get_all_view_configs()`, `save_view_config()` (busts cache). DDL added to both `schema.sql` and `migrate_schema.sql`.
...
- **Step 3: Admin UI** — 3 new routes in `admin.py`: GET /admin/views (list grouped by category), GET /admin/views/{view_key}/edit (JSON editor), POST /admin/views/{view_key} (save). Templates: `views.html` (grouped list with badges), `view_config_form.html` (JSON textarea with Format/Validate, category-specific help text). "Views" link added to sidebar between Layouts and Duplicates.
...
- **Step 4b: Dashboard Tables** — `_render_capital_raise()` loads column configs from `cr_hot_prospects_columns`, `cr_investor_breakdown_columns`, `cr_declined_columns` view configs. New `_filter_visible_columns()` helper evaluates `visible_when` conditions. Templates use `{% for col in *_columns %}` loops with `render_type` conditionals (link, badge, currency_right, mono, text, text_right).
...
- **Step 4d: Grid Defaults** — `get_default_columns()` in `grid_service.py` now checks `get_view_config(f"grid_defaults.{entity_type}")` before falling back to `_DEFAULT_COLUMNS` dict.
