...
- [x] Phase 6: Admin-Configurable View Settings — `view_configurations` table + service, seed script, admin UI (list/edit), wired to DL filter builder, dashboard tables/options, grid defaults
...
### Session 21b — March 26, 2026 (Post-Implementation Testing Fixes)
- **Bug fix: DL filter editor 500 error** — `_filter_editor.html` referenced `merged_filters` which wasn't in the template context. Fixed by computing `merged_filters` (filters + col_filters merged) in the `filter_editor` endpoint before rendering.
- **Bug fix: Drill-down column selector refreshes dashboard** — `applyColumns()` in `_column_selector.html` used `window.location` to build the HTMX URL. For drill-down grids (loaded as HTMX partials inside dashboard), `window.location` is the dashboard URL, not the drill-down URL. Fixed to use `base_url` when it contains query params (indicating a drill-down), and skip `history.pushState` for drill-down grids.
- **Enhancement: Linked field source field dropdown** — Replaced free-text input with a `<select>` populated dynamically via JS (`_SOURCE_FIELDS` dict) based on the selected source entity. Fields include all core columns per entity (org: company_name, city, country, etc.; person: name, email, phone, etc.; lead: stage, service_type, etc.).
- **Enhancement: Linked field "Direct FK" clarified** — Renamed to "Direct reference" with help text: "Use 'Direct reference' for Leads → Organization, or 'Person ↔ Organization' for People → Organization."
- **Note: Has Active Leads filter** — Server logs confirmed the boolean filter IS applying correctly (200 responses with filtered results). The grid refreshes behind the open dropdown. No code change needed.

### Session 22 — March 26, 2026 (Josh Feedback — Phase 1: Bug Fixes)
- **Source:** Josh (power user) testing session — 9 feedback items total. Phase 1 covers 2 bug fixes. Phases 2-4 cover dashboard configurability, bulk operations, and data export.
- **Bug fix: Dashboard drill-down "no records found" on column change** — Root cause: drilldown endpoints (`advisory_pipeline_drilldown`, `capital_raise_drilldown`) always returned full drilldown template (`_advisory_drilldown.html` / `_capital_raise_drilldown.html`) which wraps the grid in a header+close div. When the column selector fired an HTMX reload targeting `#drilldown-lead-grid-container`, the response nested the full wrapper inside the grid container, producing empty/broken HTML. Fixed by detecting grid-internal reloads (`visible_columns`, `sort_by`, or `page_size` in query params) and returning just `components/_grid.html` for those requests.
- **Bug fix: Distribution list dynamic filter auto-adds everyone** — Root cause: when user opened filter editor and clicked "Save Filters" without applying any column filters, `filter_criteria={}` was saved. Later `_resolve_dynamic_members({})` returned ALL non-DNC people. Fixed with: (1) server-side validation in `save_list_filters` rejecting saves with no `cf_*` keys or `q` search text, returning error HTML; (2) UI warning in `_filter_editor.html` showing amber message when no filters applied, and graying out the save button.
- Next step: Phase 2 — Dashboard Configurability (metric selector, country group-by, owner filter, traction highlighting, dashboard presets)

### Session 23 — March 26, 2026 (Josh Feedback — Phase 2: Dashboard Configurability)
- Implemented 5 dashboard configurability enhancements from Josh's feedback.
- **2A: Metric Selector on Advisory Pipeline** — Added metric selector dropdown (Lead Count / Expected Revenue / Yr1 FLAR) next to group-by. `_group_advisory_leads()` now accepts `metric` param, tracks `total_flar` alongside `total_revenue`, computes `bar_pct` based on selected metric. Bar labels change: count shows "X leads", flar shows "$X FLAR (N)", revenue shows "N leads · $X". Metric param flows through chart, drilldown, and main endpoints.
- **2B: Dashboard Presets** — Reuses `saved_views` table with `entity_type='dashboard_advisory'`. New `GET /dashboards/advisory-pipeline/presets` endpoint loads own + shared presets. "Save Preset" button on advisory pipeline captures current URL filter params (service, asset_class, owner, etc.) into `filters` JSONB and display settings (group_by, metric) into `columns` JSONB. Presets displayed as link buttons above filter bar. Save uses existing `POST /views/save` endpoint. Updated `views.py` to handle JSON columns (dict) alongside comma-separated (list) for grid views.
- **2C: Capital Raise — Country Group-By** — Added "Country" option to capital raise group-by dropdown. `_group_capital_raise_prospects()` now supports `country` case using `batch_resolve_orgs()` + `org_data.get("country")`. Country labels from `get_reference_data("country")`. Drilldown filtering by country dimension added.
- **2D: Capital Raise — Owner Filter** — Added `owner` param to `_load_capital_raise_prospects()` (DB-level `.eq("aksia_owner_id", owner)`). Owner dropdown added to capital raise template with "My Pipeline" pre-selecting current user. Owner filter persists across fund tab switches, chart group-by changes, and drilldown. All 4 capital raise endpoints accept `owner` param.
- **2E: Capital Raise — Hot Prospects (Traction Highlighting)** — Added `_TRACTION_SCORES` dict (0-5 by stage) and `_compute_traction_score()` helper. In `_render_capital_raise()`, computes traction per prospect, groups by org (highest stage, fund tickers, allocation total). Hot Prospects table shows orgs with score >= 3. Green badge for score 5 (soft circle/legal/closed), amber for 3-4 (DD/IC review). Section only renders when hot prospects exist.
- Next step: Phase 3 — Grid Bulk Operations (row checkboxes, select all, floating action bar, bulk edit/delete)

### Session 24 — March 26, 2026 (Josh Feedback — Phase 3: Grid Bulk Operations)
- Implemented 4 bulk operation features on the reusable grid component.
- **3A: Row Selection Checkboxes** — Added checkbox column as first `<th>`/`<td>` in `_grid.html`. Select-all checkbox in header toggles all rows. Individual row checkboxes fire `updateBulkBar()` to show/hide floating bar. Checkboxes hidden when `is_drilldown` is true (dashboard drilldown grids). Added `{% set is_drilldown = is_drilldown | default(false) %}` to prevent Jinja2 undefined errors on non-drilldown grids. Colspan calculations updated for checkbox column.
- **3B: Floating Bulk Action Bar** — Fixed-position bar at bottom of viewport appears when 1+ rows selected. Shows "X selected" count, "Edit Selected" (brand-500), "Delete Selected" (red-500), and "Clear" buttons. Select-all checkbox supports indeterminate state when partial selection.
- **3C: Bulk Edit Modal + Endpoint** — Modal with field selector dropdown (populated from visible columns, excludes id/created_at/updated_at/created_by/linked fields). Value input dynamically changes by field type: dropdowns show options from reference_data, booleans show Yes/No, dates show date picker, numbers show number input. `POST /views/bulk-edit/{entity_type}` endpoint: iterates record IDs, fetches old record, coerces value by field type, handles core vs EAV split via `split_core_eav` pattern, writes audit log per record. Permissions: admin, standard_user, rfp_team.
- **3D: Bulk Delete (Soft Delete)** — `POST /views/bulk-delete/{entity_type}` endpoint: admin-only, sets `is_deleted=True` per record with audit logging. JS `bulkDelete()` shows confirmation dialog with record count before POST. Both bulk endpoints return count of affected records.
- All JS uses `fetch()` for async POST, dispatches `gridRefresh` custom event on success (reuses existing HTMX listener on grid container). Selection cleared after successful operations.
- Next step: Phase 4 — Data Export to Excel

### Session 25 — March 26, 2026 (Josh Feedback — Phase 4: Data Export to Excel)
- Implemented Excel export feature for all grid entities.
- **4A: Export Button + Endpoint** — Added "Export" button to grid toolbar (download icon, plain `<a>` tag). `GET /views/export/{entity_type}` endpoint reuses `build_grid_context()` with new `export_mode=True` flag that bypasses the page_size cap (50K rows). Uses openpyxl to build `.xlsx` workbook with: bold headers, type-aware cell formatting (currency `#,##0`, dates as native Excel dates, booleans as Yes/No, dropdowns title-cased, multi_select/text_list comma-joined). Enriched display values for FK fields (org names, owner names, fund tickers instead of UUIDs). Auto-sized columns from first 50 rows. Returns `StreamingResponse` with timestamped filename.
- **4B: Export Selected** — "Export Selected" button (green) added to the floating bulk action bar. JS `exportSelected()` builds export URL with `record_ids=id1,id2,...` query param. Endpoint filters rows to only the selected IDs after running the grid query.
- **grid_service.py change:** Added `export_mode` parameter to `build_grid_context()`. When True, sets `page=1, page_size=50000` instead of the normal 100-row cap.
- **requirements.txt:** Added `openpyxl==3.1.5`.
- Export button hidden on drilldown grids (consistent with checkbox/bulk bar behavior).

### Session 26 — March 26, 2026 (Josh Feedback — Bug Fix Round)
- Fixed 8 bugs found during Josh testing of Phases 1-4.
- **Presets 500 TypeError** — `columns` in saved_views can be a list (grid screeners) or dict (dashboard presets). `{**columns}` crashed on lists. Added `isinstance(columns, dict)` guard in `advisory_pipeline_presets`.
- **Drilldown URL corruption** — Two separate issues: (1) Grid template used `{{ base_url }}?{{ filter_qs }}` producing double `?` for drilldown URLs. Fixed with `url_sep` variable. (2) Column selector's JS string `var baseUrl = '{{ base_url }}'` was HTML-escaped by Jinja2 (`&` → `&amp;`), corrupting URLs for HTMX requests. Fixed with `{{ base_url | safe }}`.
- **Advisory defaults** — Changed default metric from "revenue" to "count" across all 3 advisory endpoints. Non-stage group-bys now sort descending by selected metric.
- **Group-by/metric reset on filter change** — Top filter dropdowns reload `_advisory_content.html` which recreates chart dropdowns with defaults. Added hidden inputs for `metric` and `group_by` in the filter form, plus `onchange` handlers on chart dropdowns that sync values to hidden inputs. Added `group_by` parameter to main advisory pipeline endpoint so it passes through.
- **Bulk edit lookup fields** — `coverage_owner` (UUID FK) sent display name "Melissa" instead of UUID. Added `lookup` case in `onBulkFieldChange()` rendering a user select dropdown. Embedded `window._bulkEditUsers` from grid context.
- **Bulk edit empty dropdown options** — `dropdown_category` not set on some field_definitions. Added fallback in `enrich_field_definitions` to try `field_name` as reference_data category.
- **Preset delete** — Added delete buttons (red X on hover) for own presets in `_advisory_presets.html`.
- **`_load_advisory_leads` fixes** — Added `lead_type` to SELECT and `.or_("lead_type.eq.advisory,lead_type.is.null")` filter. Fixed `_apply_filters` in grid_service.py for advisory lead_type to include NULLs. Fixed `_resolve_dynamic_members` to return empty list on empty filter criteria.
- Outstanding: DL filter editor needs redesign toward filter-builder UX; capital raise active default + hot prospects tweaks.

### Session 27 — March 26, 2026 (Josh Feedback — Phase 5: Outstanding Fixes)
- Implemented 3 outstanding items from Josh's feedback testing.
- **5C: Advisory Pipeline — Drilldown Header Label Fix** — Both advisory and capital raise drilldown endpoints resolved value labels with naive `value.replace("_", " ").title()`, which didn't handle UUIDs (owner, fund) or reference_data keys (stage, service_type, etc.). Created shared `_resolve_drilldown_value()` helper that resolves labels by dimension type: owner→`batch_resolve_users`, fund→funds query, stage/service_type/asset_class/lp_type/country→reference_data labels, generic fields→reference_data by field name.
- **5B.1: Capital Raise — Active Filter Default** — Added `FP_INACTIVE_STAGES = {"closed", "declined"}` constant. Added `active_filter` param (default "active") to all 4 capital raise endpoints + `_render_capital_raise()`. Active filter excludes closed+declined by default. Added Status dropdown to template (Active Only / All / Closed+Declined). Filter persists across fund tab switches, chart group-by changes, and drilldown. `crNavigate()` JS helper preserves all filter params across navigation.
- **5B.2: Chart Sort** — Already implemented correctly. No change needed.
- **5B.3: Hot Prospects Tweaks** — Removed Fund(s) column from hot prospects table. Added Lead Owner column with `aksia_owner_id` resolved via `batch_resolve_users()` (uses owner from the highest-stage prospect per org). Added client-side JS filter dropdowns (Lead Owner + Stage) above the table with `filterHotProspects()` using `data-owner`/`data-stage` attributes on rows.
- **5A: Distribution List Filter Editor Redesign** — Complete rewrite from grid-distill approach to filter-BUILDER UX (similar to HubSpot segments). New filter criteria format: `{"filters": [{"field": "country", "operator": "in", "value": ["US", "GB"]}, ...]}` stored in existing `filter_criteria` JSONB column. New constants: `_OPERATORS_BY_TYPE` (12 field types × appropriate operators), `_ORG_FILTER_FIELDS` (4 org virtual fields). New helpers: `_build_filter_fields()` loads person field_definitions + org virtual fields with enriched dropdown options; `_convert_old_to_new_format()` converts legacy `cf_*` format. `_resolve_dynamic_members()` now detects format and dispatches to `_resolve_dynamic_members_old()` (backward compat) or `_resolve_dynamic_members_new()` (new format). New `_apply_filter_to_query()` maps 12 operators to Supabase query methods. New `POST /preview-filter-count` endpoint for live match count. Template rewritten with inline JS filter builder: `addFilterRow()`, `onFieldChange()`, `onOperatorChange()`, dynamic value inputs (text/select/multi-checkbox/number/date), debounced preview via fetch, `saveFilters()` via JSON POST.

### Session 28 — March 27, 2026 (Phase 6: Admin-Configurable View Settings)
- Implemented Phase 6: admin-configurable view settings so admins can customize filters, table columns, and dropdown options without code changes.
- **Step 1: Schema + Service** — New `view_configurations` table (view_key UNIQUE, display_name, description, category, config JSONB, updated_by). New `db/view_config_service.py` with `get_view_config()` (TTL-cached 60s), `get_all_view_configs()`, `save_view_config()` (busts cache + audit logs). DDL added to both `schema.sql` and `migrate_schema.sql`.
- **Step 2: Seed Script** — `scripts/seed_view_configurations.py` seeds 13 rows with current hardcoded defaults. Supports `--force` to overwrite. Categories: distribution_lists (1), dashboards (5), grids (7).
- **Step 3: Admin UI** — 4 routes in `admin.py`: GET /admin/views (list grouped by category), GET /admin/views/{view_key}/edit (visual builder), POST /admin/views/{view_key} (save with validation), POST /admin/views/{view_key}/reset (reset to seed default). Templates: `views.html` (grouped list with badges), `view_config_form.html` (4 visual editors: column builder, option list builder, grid column picker, DL filter editor + JSON fallback). "Views" link added to sidebar between Layouts and Duplicates.
- **Step 4a: DL Filter Builder** — `_build_filter_fields()` now reads `dl_filter_fields` view config for person field whitelist, org virtual fields, and filterable field types. Added `"lookup"` operator type to `_OPERATORS_BY_TYPE`. Lookup fields load active users as dropdown options.
- **Step 4b: Dashboard Tables** — `_render_capital_raise()` loads column configs from `cr_hot_prospects_columns`, `cr_investor_breakdown_columns`, `cr_declined_columns` view configs. New `_filter_visible_columns()` helper evaluates `visible_when` conditions. Templates use `{% for col in *_columns %}` loops with `render_type` conditionals (link, badge, currency_right, mono, text, text_right). Data dicts dynamically include ALL org and lead fields via `_enrich_with_org_fields()` / `_enrich_with_lead_fields()` with reference_data label resolution.
- **Step 4c: CR Group-By + Advisory Metrics** — Capital raise group-by options loaded from `cr_group_by_options` view config. Advisory metric selector loaded from `advisory_metric_options` view config. Both template dropdowns now iterate config options instead of hardcoded `<option>` tags.
- **Step 4d: Grid Defaults** — `get_default_columns()` in `grid_service.py` now checks `get_view_config(f"grid_defaults.{entity_type}")` before falling back to `_DEFAULT_COLUMNS` dict. Grid editor shows virtual columns + admin-created linked fields alongside core fields.
